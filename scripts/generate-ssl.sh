...
echo -e "${CYAN}Generating SSL certificate for api.test.local and app.test.local...${NC}"
cd "$SSL_DIR"
mkcert api.test.local app.test.local localhost 127.0.0.1 ::1

# Rename to consistent names used by nginx.conf
mv "api.test.local+4.pem" "test.local.pem" 2>/dev/null || true
mv "api.test.local+4-key.pem" "test.local-key.pem" 2>/dev/null || true

echo ""
echo -e "${GREEN}SSL certificates generated successfully!${NC}"
echo -e "  Certificate: $SSL_DIR/test.local.pem"
echo -e "  Key: $SSL_DIR/test.local-key.pem"
echo ""
echo -e "${YELLOW}Add to /etc/hosts:${NC}"
echo -e "${CYAN}  127.0.0.1 api.test.local app.test.local${NC}"
echo ""
