...
test.describe("Materials Module", () => {
    let categoryId;
    let materialTypeId;
    test("POST /materials/categories - should create category", async ({ request, }) => {
    ...
    test("GET /materials/categories - should list categories", async ({ request, }) => {
        const res = await request.get("materials/categories");
        expect(res.status()).toBe(200);
    });
    // Dependent on category
    test("POST /materials/types - should create material type", async ({ request, }) => {
        if (!categoryId)
            test.skip();
    ...
    // Dependent on type
    test("POST /materials/instances - should create instance", async ({ request, }) => {
        if (!materialTypeId)
            test.skip();
...
//# sourceMappingURL=materials.spec.js.map